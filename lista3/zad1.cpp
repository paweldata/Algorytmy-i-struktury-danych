...
void radixSort(int* tab, int size, bool ascending);
void countingSort(int* tab, int* temp, int size, int place, bool ascending);
...
void radixSort(int* tab, int size, bool ascending) {
    ...
    fprintf(stderr, "find max value = %d\n", max);

    for (int i = 1; max / i > 0; i*= 10)
        countingSort(tab, temp, size, i, ascending);

    free(temp);
}

void countingSort(int* tab, int* temp, int size, int place, bool ascending) {
    fprintf(stderr, "do countingSort for place = %d\n", place);
    int count[10] = {0};
    int index;

    for (int i = 0; i < size; i++) {
        index = ascending ? (tab[i] / place) % 10 : 9 - (tab[i] / place) % 10;
        count[index]++;
        setValueAmount++;
    }
...
void sortTable(int size, bool ascending, void(*sortFunction)(int *, int, bool)) {
    ...
    if (setValueAmount > 0)
        fprintf(stderr, "Ilosc przypisan : %d\n", setValueAmount);
...
void makeStatistics(FILE* file, int testNumber, bool ascending, void(*sortFunction)(int *, int, bool)) {
    ...
    std::uniform_int_distribution<> randInt(100000000, 1000000000);
    ...
    for (int size = 100; size <= 10000; size += 100) {
        ...
        for (int count = 0; count < testNumber; count++) {
            ...
            for (int i = 0; i < size - 1; i++)
             ...
             fprintf(file, "%d\t%d\t%f\n", size, std::max(compareAmount + swapAmount, setValueAmount), time * 1000);
        }
...
int main(int argc, char *argv[]) {
    ...
    if (sortFunction == nullptr) {
        printf("Give sort type as argument\n"
               "--type quick | insert | merge | dualQuick | hybrid | radix\n"
               "--comp <= | >=\n"
